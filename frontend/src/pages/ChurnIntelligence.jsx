...
const RISK_COLORS = { high: '#EF4444', medium: '#4F46E5', low: '#22C55E' };
...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div style={{ background: 'var(--text-main)', padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(239,68,68,.25)', fontSize: 12, color: 'var(--bg-card)' }}>
      <p style={{ fontWeight: 700, color: '#4F46E5', marginBottom: 4 }}>{d.name}</p>
      ...
      <main className="xn-page-content">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 22 }}>
          {[
            ...
            { label: 'Medium Risk', count: medium.length, color: '#4F46E5', bg: 'rgba(79,70,229,.07)',  desc: 'Churn score 40–75 — Monitor closely',           key: 'medium' },
              ...
              <BarChart data={buckets} barSize={28}>
                ...
                <Bar dataKey="count" radius={[4,4,0,0]}>
                  {buckets.map((b, i) => {
                    ...
                    return <Cell key={i} fill={midScore > 75 ? '#EF4444' : midScore >= 40 ? '#4F46E5' : '#22C55E'} />;
                  })}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="xn-card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-light)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={() => setSelectedRisk('all')} style={{ fontSize: 11, color: '#4F46E5', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>
