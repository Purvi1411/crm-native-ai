...
const Toggle = ({ value, onChange }) => (
  <button onClick={() => onChange(!value)} style={{
    ...
    background: value ? '#06B6D4' : '#E5E7EB', position: 'relative', transition: 'background .2s', flexShrink: 0
  }}>
    <span style={{
...
export default function Settings() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (!token) return;
        const { data } = await axios.get('https://crm-native-ai-1.onrender.com/api/auth/me', {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (data.settings && Object.keys(data.settings).length > 0) {
          setSettings(data.settings);
          localStorage.setItem('xeno_settings', JSON.stringify(data.settings));
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (token) {
        await axios.put('https://crm-native-ai-1.onrender.com/api/auth/settings', settings, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
    ...
    <div className="xn-app-layout">
      ...
      <main className="xn-page-content">
        <div className="xn-page-header">
          ...
          <button className="xn-btn-primary" onClick={handleSave} disabled={saving}>
            ...
            <Section title="🤖 AI Configuration">
              ...
              <SettingRow label="AI Temperature" desc="Higher = more creative, Lower = more precise">
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <input type="range" min="0" max="1" step="0.1" value={settings.temperature} onChange={e => set('temperature', e.target.value)} style={{ width: 100, accentColor: '#06B6D4' }} />
                  <span style={{ fontSize: 12, fontWeight: 700, color: '#06B6D4', width: 28, textAlign: 'right' }}>{settings.temperature}</span>
                </div>
              </SettingRow>
            </Section>
          </div>
          ...
          <div>
            ...
            <Section title="📢 Channel Toggles">
              {[
                ...
                { key: 'smsEnabled',      label: 'SMS',       color: '#06B6D4', icon: '📱' },
