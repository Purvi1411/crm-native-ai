...
export default function Login() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setError(!err.response ? 'Cannot reach server. Is the backend running?' : err.response?.data?.message || 'Invalid email or password.');
        ...
        <div className="xp-right">
          ...
          <div style={{ width:'100%', maxWidth:360, animation:'fadeUp .6s ease both .15s' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 36 }}>
              ...
              <h2 style={{ fontSize:32, fontWeight:800, color:'var(--xp-text)', letterSpacing:'-0.5px', marginBottom:10 }}>Welcome to <span style={{ color:'#06B6D4' }}>Xeno</span></h2>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="xp-field">
                ...
                <div className="xp-iw">
                  ...
                  <input id="xp-pwd" type={showPwd?'text':'password'} name="password" className="xp-inp" value={form.password} onChange={handleChange} placeholder="Enter your password" required autoComplete="current-password"/>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', margin:'16px 0 28px' }}>
                <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:15, color:'var(--xp-text-secondary)', cursor:'pointer', userSelect:'none' }}>
                  <input type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)} style={{ accentColor:'#06B6D4', width:17, height:17 }}/>
                  Remember me
                </label>
                <Link to="/forgot-password" style={{ fontSize:15, color:'#06B6D4', fontWeight:600, textDecoration:'none', transition:'opacity .2s' }}
              ...
              <button type="submit" className="xp-btn" disabled={loading}>
                {loading ? <><span className="xp-spinner"/>Signing in…</> : 'Login'}
              </button>
            </form>
            ...
            <p style={{ textAlign:'center', marginTop:28, fontSize:16, color:'var(--xp-text-faint)' }}>
              New to Xeno?{' '}
              <Link to="/register" style={{ color:'#06B6D4', fontWeight:700, textDecoration:'none', transition:'opacity .2s' }}
                onMouseEnter={e=>e.target.style.opacity='0.8'} onMouseLeave={e=>e.target.style.opacity='1'}>
                Create an account →
              </Link>
            </p>
