    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div style={{ background: 'var(--text-main)', color: 'var(--bg-card)', padding: '10px 14px', borderRadius: 10, fontSize: 12, boxShadow: '0 8px 24px rgba(0,0,0,.2)', border: '1px solid rgba(79,70,229,.2)' }}>
          <p style={{ fontWeight: 700, color: '#06B6D4', marginBottom: 5 }}>{d.name}</p>
  ...
  return (
    <div style={{ padding: '4px 4px 0' }}>
          ...
          <ScatterChart margin={{ top: 10, right: 20, bottom: 10, left: 20 }}>
            ...
            <Scatter name="Customers" data={chartData}>
              {chartData.map((entry, i) => {
                const color = entry.churnRiskScore > 75 ? '#EF4444' : entry.churnRiskScore >= 40 ? '#06B6D4' : '#22C55E';
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 8, justifyContent: 'flex-end' }}>
        {[['#22C55E', 'Low Risk'], ['#06B6D4', 'Medium Risk'], ['#EF4444', 'High Risk']].map(([c, l]) => (
