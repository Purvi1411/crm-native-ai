...
const GeoMap = ({ customers }) => {
  ...
  useEffect(() => {
    ...
    customers.forEach((c) => {
      const loc = c.country === "India" && c.state ? c.state : null;
      if (!loc) return;
      if (!data[loc]) {
  ...
  const getLocKey = (geo) => {
    const geoName = geo.properties.NAME_1 || geo.properties.NAME || geo.properties.name || "";
      ...
      <Geographies geography={geoUrlIndia}>
        {({ geographies }) =>
          geographies.map((geo, geoIndex) => {
            ...
            // Dot radius: proportional to count, min 8 max 22
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--text-main)", display: "flex", alignItems: "center", gap: 8, margin: 0 }}>
            <span style={{ display: "inline-block", width: 10, height: 10, borderRadius: "50%", background: "linear-gradient(135deg,#6366F1,#EC4899)", boxShadow: "0 0 8px rgba(99,102,241,0.6)" }} />
            India Customer Map
          </h2>
          <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "4px 0 0" }}>
            {customers.filter((c) => c.country === "India").length} customers across{" "}
            {Object.keys(locData).length} states &nbsp;·&nbsp;
